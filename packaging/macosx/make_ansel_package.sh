#!/bin/bash
#
# Copied from https://github.com/darktable-org/darktable/blob/master/packaging/macosx/3_make_hb_darktable_package.sh
#
...
dtExecutables=$(echo "$dtExecDir"/ansel{,-cli,-cltest,-generate-cache,-rs-identify,-curve-tool,-noiseprofile})
...
function install_dependencies {
    ...
    absolutePath=$(dirname $(./realpath.py "$1"))
...
function reset_exec_path {
    ...
    if [[ "$oToolLDependencies" == *"@rpath/libansel.dylib"* && "$1" != *"libansel.dylib"* ]]; then
        # Only need to reset binaries that live outside of lib/ansel
        oToolLoader=$(otool -l "$1" 2>/dev/null | grep '@loader_path' | cut -d\( -f1 | sed 's/^[[:blank:]]*path[[:blank:]]*//;s/[[:blank:]]*$//' ) || true
        if [[ "$oToolLoader" == "@loader_path/../lib" ]]; then
            echo "Resetting loader path for libansel.dylib of <$1>"
            install_name_tool -rpath @loader_path/../lib @loader_path/../Resources/lib "$1" || true
        fi
    fi
    ...
    if [[ -n "$hbDependencies" && "$hbDependencies" != "" ]]; then
        ...
        echo "Resetting executable path of <$1>"
        ...
        for hbDependency in $hbDependencies; do
            ...
            echo "   $hbDependency => @executable_path/../Resources/lib/$dynDepOrigFile"
            ...
            if [[ -n "$oToolLoader" ]]; then
                echo "   Resetting loader path for dependency <$hbDependency>"
                oToolLoaderNew=$(echo $oToolLoader | sed "s#@loader_path/##" | sed "s#../../../../opt/.*##")

                echo "      $oToolLoader => @loader_path/${oToolLoaderNew}${dynDepOrigFile}"
...
if [ -n "$CODECERT" ]; then
    ...
    find ${dtWorkingDir}/Contents/Resources/lib -type f -exec codesign --verbose --force --options runtime -i "org.ansel" -s "${CODECERT}" \{} \;
    codesign --deep --verbose --force --options runtime -i "org.ansel" -s "${CODECERT}" ${dtWorkingDir}
else
    # Use ad-hoc signing and preserve metadata
    find ${dtWorkingDir}/Contents/Resources/lib -type f -exec codesign --verbose --force --preserve-metadata=entitlements,requirements,flags,runtime -i "org.ansel" -s - \{} \;
    codesign --deep --verbose --force --preserve-metadata=entitlements,requirements,flags,runtime -i "org.ansel" -s - ${dtWorkingDir}
fi
